...
def likelihood(x, n, p):
    ...
    of obtaining p is a 1D numpy.ndarray
    """
    ...
    if x > n:
        raise ValueError("x cannot be an integer greater than n")

    if not isinstance(p, np.ndarray) or len(p.shape) != 1:
        raise TypeError("P must be a 1D numpy.ndarray")

    if np.any(p > 1) or np.any(p < 0):
        raise ValueError("All the values in P must be in the range [0, 1]")
